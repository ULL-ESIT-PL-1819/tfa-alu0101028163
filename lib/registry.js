...
specialForms['object'] = function(args, env){
  // const object_name = args[0].value;
  let object_properties = new function(){};
  for(let i = 0; i < args.length; i++){
    const arg = args[i];
    const arg_id = arg.args[0].value;
    let arg_value;
    if(arg.args[1].hasOwnProperty('operator') && arg.args[1].operator.value === 'do'){        // It's a procedure.
      arg_value = arg.args[1];
    }else arg_value = arg.args[1].evaluate(env); 
    object_properties[arg_id] = arg_value;
  }
  
  // env[object_name] = object_properties;
