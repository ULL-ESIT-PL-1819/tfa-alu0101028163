...
function parse_block(){
  ...
  const classes = [];
  let class_;
  while(lookahead && (class_ = parse_class())){
    classes.push(class_);
  }
  ...
  block_statements = []
  if(classes)    block_statements.push(...classes);
...
function parse_class(){
  if(lookahead && lookahead.value === 'class' && lookahead.type === 'keyword'){
    get_token("class","keyword");
    const id = get_token_of_type('identifier');
    
    
    let class_apply = null;
    if (lookahead.value === 'extends'){
      get_token("extends","keyword");
      const parent_class = get_token_of_type('identifier');
      class_apply = egg_apply("extends", parent_class);
    }
    ...
    if(class_apply){
      class_apply.args.push(egg_apply("class", id, ...attributes));
      return class_apply;
    }else return egg_apply("class", id, ...attributes);
  }else return null;
}
...
function parse_assign_stmt(){
  if(lookahead.type === 'identifier'){
    ...
    let right_part = parse_object_inst();
    if(!right_part){
...
function parse_object_inst(){
  if(lookahead && lookahead.value === 'new'){
    get_token("new", "keyword");
    const class_name = get_token_of_type('identifier');
    return egg_apply("new",class_name);
  }return null;
}
